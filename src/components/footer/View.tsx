...
export const Footer: React.FC<{ data: FooterData; settings: FooterSettings }> = ({ data }) => {
  ...
  const socialLinks = Array.isArray(data.socialLinks) ? data.socialLinks.filter((link) => link.id) : [];
                ...
                {navItems.map((item) => (
                  <li key={item.id} data-jp-item-id={item.id} data-jp-item-field="menu">
                    {isInAppPathHref(item.href) ? (
                      ...
                      <a href={item.href} className="text-[var(--local-text-muted)] transition hover:text-[var(--local-primary)]" data-jp-field="href">
                        <span data-jp-field="label">{item.label}</span>
                      </a>
                    )}
                  </li>
          ...
          <div className="flex space-x-4">
            {socialLinks.map((link) => (
              <a 
                ...
                data-jp-field="url"
              >
                <span className="sr-only" data-jp-field="platform">{link.platform}</span>
